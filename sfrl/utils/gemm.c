/** 快速矩阵乘法，本质是通过对对访存的优化，降低缺页率提高计算效率 算法详见
 *  http://apfel.mathematik.uni-ulm.de/~lehn/sghpc/gemm/page02/index.html
 * 此处使用的是4*4的版本
 * https://github.com/flame/how-to-optimize-gemm/blob/master/src/MMult_4x4_9.c
 **/
...
void Gemm(int m, int n, int k, float *a, int lda, float *b, int ldb, float *c,
          int ldc) {
  int i, j;
