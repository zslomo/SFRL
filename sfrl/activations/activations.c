// code copy from darknet
// https://github.com/pjreddie/darknet/tree/master/src/activations.c
...
char *get_activation_string(ActiType a)
{
    switch(a){
        case LOGISTIC:
            return "logistic";
...
*/
ActiType get_activation(char *s)
{
...
**       a    激活函数类型
*/
float activate(float x, ActiType a)
{
    switch(a){
        case LINEAR:
...
*/
void activate_array(float *x, const int n, const ActiType a)
{
    int i;
    // 逐个处理x中的元素
    for(i = 0; i < n; ++i){
...
** 输出： 激活函数关于输入x的导数值
*/
float gradient(float x, ActiType a)
{
...
*/
void gradient_array(const float *x, const int n, const ActiType a, float *delta)
{
    int i;
    for(i = 0; i < n; ++i){
        // printf("x[i] = %f, delta = %f, gradient = %f \n", x[i], delta[i], gradient(x[i], a));
